...
export default function MyStats() {
  ...
  const { data: analytics, loading } = useFetch(
    () => getEmployeeAnalytics(user.id, period),
    [user?.id, period]
  );
  const { data: screenshots } = useFetch(() => getScreenshots(user.id), [user?.id]);
  const { data: events } = useFetch(() => getEvents(user.id), [user?.id]);

  if (loading) return <Spinner />;

  const pieData = analytics?.app_breakdown?.map((a) => ({
    name: a.app.slice(0, 20),
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        <h2 className="text-xl font-bold text-white flex-1">My Activity — {user?.name}</h2>
      ...
      <Card>
        <h3 className="text-white font-semibold mb-4">App Breakdown</h3>
        {pieData.length > 0 ? (
      ...
      <Card>
        <h3 className="text-white font-semibold mb-3">My Screenshots</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          {!screenshots?.length && <p className="text-slate-500 text-sm col-span-4">No screenshots yet</p>}
        </div>
      </Card>
    </div>
  );
